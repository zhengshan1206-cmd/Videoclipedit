...
class AiFilesDownoadDialog extends StatefulWidget {
  ...
  final String? toast;
  final void Function(int, String)? onSuccess;

  const AiFilesDownoadDialog({
    super.key,
  ...
  @override
  State<AiFilesDownoadDialog> createState() => _AiFilesDownoadDialogState();
}

class _AiFilesDownoadDialogState extends State<AiFilesDownoadDialog> {
  ...
  void _downloadFiles() async {
    ...
    await _downloadManager.downloadMultipleFiles(
      widget.fileUrls,
      ...
      onSuccess: (index, path) {
        widget.onSuccess?.call(index, path);
      },
    );
                  ...
                  children: [
                    ...
                    Padding(
                      ...
                      child: ByWidgetsUtil.commonText(
                        ...
                        text:
                            "${widget.fileUrls.length == 1 ? "下载中\n当前进度:${(_currentProgress * 100).toStringAsFixed(0)}%" : "第$_currentVideoIndex个(共${widget.fileUrls.length}个)文件下载中\n当前进度:${(_currentProgress * 100).toStringAsFixed(0)}%"}\n如果提前离开将取消下载。",
                      ),
                    ),
