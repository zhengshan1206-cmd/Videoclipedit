...
import 'package:video_clip_edit/v2/aiSquare/cartoon/widgets/ai_cartoon_input_view.dart';
import 'package:video_clip_edit/v2/aiSquare/cartoon/widgets/ai_cartoon_batch_replace_dailog.dart';

class AiCartoonProhibitedWordsDailog<T extends AiSettingsMixin>
  ...
  @override
  State<AiCartoonProhibitedWordsDailog> createState() =>
      _AiCartoonProhibitedWordsDailogState<T>();
}

class _AiCartoonProhibitedWordsDailogState<T extends AiSettingsMixin>
    extends State<AiCartoonProhibitedWordsDailog<T>> {
  ...
  void initState() {
    ...
    wordsEditingController = TextEditingController(text: provider.desc);

    // 对话框打开时，如果检测到违禁词，自动执行首字母替换
    // WidgetsBinding.instance.addPostFrameCallback((_) {
    //   if (provider.bandedWords.isNotEmpty) {
    //     provider.updateDesc(provider.replaceWithInitialLetterOfPinyin());
    //     provider.detect(context, provider.desc);
    //   }
    // });
  }
            ...
            Container(
              ...
              child: Column(
                ...
                children: [
                  ...
                  SizedBox(
                    ...
                    child: ByWidgetsUtil.commonBtn(
                      ...
                      onClick: () {
                        ...
                        final provider = context.read<T>();
                        if (lastDetectedContent == provider.desc) {
                  ...
                  if (provider.bandedWords.isEmpty) const SizedBox(height: 10),
                  if (provider.bandedWords.isEmpty)
  ...
  void _copyContent(BuildContext context) {
    Clipboard.setData(
      ClipboardData(
        text: context.read().subtitlesBean.wordsOrigin,
      ),
    );
  ...
  _buildActions(BuildContext context) {
    return Offstage(
      offstage: context.watch<T>().bandedWords.isEmpty,
      child: Container(
            ...
            Expanded(
              child: ByWidgetsUtil.commonBtn(
                ...
                onClick: () {
                  final provider = context.read<T>();
                  provider
                      .updateDesc(provider.replaceWithInitialLetterOfPinyin());
                  provider.detect(context, provider.desc);
                },
              ),
            ),
                ...
                onClick: () {
                  ...
                  showDialog(
                    ...
                    builder: (ctx) => ChangeNotifierProvider.value(
                      ...
                      child: AiCartoonBatchReplaceDailog<T>(),
                    ),
                  );
                },
              ),
