...
enum AiCartoonVideoManagementPageType { clip, normal }

class AiCartoonVideoManagementPage extends StatefulWidget {
  const AiCartoonVideoManagementPage({
    super.key,
  ...
  @override
  State<AiCartoonVideoManagementPage> createState() =>
      _AiCartoonVideoManagementPageState();
}

class _AiCartoonVideoManagementPageState
    extends State<AiCartoonVideoManagementPage> {
  final tips =
  ...
  String _getPageTag() {
    ...
    if (widget.type == AiCartoonVideoManagementPageType.normal) {
      return "myworks_list_novel_tweets_video_works";
    } else if (widget.source == EntranceSource.explosive) {
      return "myworks_list_explosive_video_works";
    } else if (widget.source == EntranceSource.shortPlay) {
  ...
  Widget build(BuildContext context) {
    final isRefreshing = context.select<AiCartoonVideoManagementProvider, bool>(
      (p) => p.isRefreshing,
    );
    final editing = context.select<AiCartoonVideoManagementProvider, bool>(
      (p) => p.videosEditing,
    );
    return Scaffold(
  ...
  _buildBottmBar(BuildContext context) {
    final provider = context.read<AiCartoonVideoManagementProvider>();
  ...
  List<Widget> _buildActions(BuildContext context) {
    ...
    final worksEditing = context
        .select<AiCartoonVideoManagementProvider, bool>((p) => p.videosEditing);
    final selectAll = context
        .select<AiCartoonVideoManagementProvider, bool>((p) => p.selectAll);
    if (worksEditing) {
      return [
        GestureDetector(
    ...
    return [
      Offstage(
        offstage: context
            .select<AiCartoonVideoManagementProvider,
  ...
  FutureOr _loadVideos({
    ...
    provider.loadVideoList(
      videoQueryType:
          widget.type == AiCartoonVideoManagementPageType.clip ? 1 : 2,
      source: widget.source,
    );
  }
}
...
class VideoManagementView extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final times = context.select<AiCartoonVideoManagementProvider, int>(
      (p) => p.times,
    );
    ...
    return CustomScrollView(
      slivers: [
        ...
        SliverPadding(
          ...
          sliver: AiCartoonVideoManagementSliverListView(
            type: type,
            videoQueryType:
                type == AiCartoonVideoManagementPageType.clip ? 1 : 2,
            source: source,
          ),
        ),
      ],
