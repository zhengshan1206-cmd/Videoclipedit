...
///推广-热门小说
class PromoteHotNovelsPage extends StatefulWidget {
  const PromoteHotNovelsPage({
    super.key,
  ...
  @override
  State<PromoteHotNovelsPage> createState() => _PromoteHotNovelsPageState();
}

class _PromoteHotNovelsPageState extends State<PromoteHotNovelsPage>
    with AutomaticKeepAliveClientMixin {
  final controller = Get.find<PromoteHotNovelsController>();
  ...
  void _checkAndReportBanner() {
    if (_hasBanner() && controller.isShowBanner) {
      ByNavigatorUtil.reportDataPoint(
        ...
        extra: {"position": "promote_hot_novels"},
      );
    }
  }
  ...
  Widget build(BuildContext context) {
    super.build(context);
    return GetBuilder<PromoteHotNovelsController>(builder: (controller) {
      return VisibilityDetector(
        key: Key('promote_hot_novels_page_${widget.categoryBean.id}'),
          ...
          child: MultiStatusView(
              ...
              child: ListView.builder(
                ...
                itemBuilder: (context, index) {
                  ...
                  final novelsBean = controller.hotNovelsList[validIndex];
                  return NovelListCell(
                    novelBean: novelsBean,
                    index: validIndex,
                    promoteType: PromotionCategoryType.hotNovel,
                  );
                },
              )),
        ),
  ...
  _getItemCount() {
    ...
    return controller.hotNovelsList.length + bannerLenght + loadMoreLength;
  }
