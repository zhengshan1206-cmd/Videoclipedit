...
///推广-民间故事controller
class PromoteFolkStoryController extends BaseController {
  var multiStatus = MultiStatusType.statusLoading.obs;

  /// 民间故事列表
  var folkStoryList = <HotCreateNovelBean>[];
  ...
  ///查询民间故事列表
  void getFolkStoryList(bool isRefresh) async {
    if (isRefresh) {
    ...
    HttpUtils.get(
      ...
      success: (data) {
        ...
        List<HotCreateNovelBean> storyBeans =
            listData.map((e) => HotCreateNovelBean.fromJson(e)).toList();

        multiStatus.value = MultiStatusType.statusContent;
        if (isRefresh) {
          folkStoryList.clear();
        }
        folkStoryList.addAll(storyBeans);
        if (folkStoryList.isEmpty) {
          multiStatus.value = MultiStatusType.statusEmpty;
        }
        pageHelper.addPage();

        final hasMore = storyBeans.length < pageHelper.row ? false : true;
