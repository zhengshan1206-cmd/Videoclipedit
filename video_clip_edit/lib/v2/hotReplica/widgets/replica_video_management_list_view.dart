...
class ReplicaVideoManagementListView extends StatefulWidget {
  const ReplicaVideoManagementListView({
    super.key,
  });

  @override
  State<ReplicaVideoManagementListView> createState() =>
      _AiOralVideoManagementListViewState();
}

class _AiOralVideoManagementListViewState
    extends State<ReplicaVideoManagementListView> {
  @override
  Widget build(BuildContext context) {
    ...
    return SliverGrid.builder(
      ...
      itemBuilder: (context, index) => ReplicaVideoManagementListViewCell(
        index: index,
...
extension AiCartoonVideoStatusExt on ReplicaVideoStatus {
  int get rawValue {
    switch (this) {
...
class ReplicaVideoManagementListViewCell extends StatelessWidget {
  const ReplicaVideoManagementListViewCell({
    super.key,
    required this.index,
    required this.bean,
  });
  ...
  Widget build(BuildContext context) {
    ...
    final videosEditing = context.select<ReplicaVideoManagementProvider, bool>(
        (value) => value.videosEditing);

    final selectedVideoIdxs =
        context.select<ReplicaVideoManagementProvider, List<int>>(
    ...
    return GestureDetector(
      ...
      onTap: () {
        if (videosEditing) {
          context
              .read<ReplicaVideoManagementProvider>()
          ...
          if (status == ReplicaVideoStatus.finished) {
            ByNavRouterUtils.push(
                ...
                AiCartoonVideoPreviewPage(
                  videoBean: bean,
                  showCopyBtn: false,
                  provider2: context.read<ReplicaVideoManagementProvider>(),
                ));
          }
        }
      },
