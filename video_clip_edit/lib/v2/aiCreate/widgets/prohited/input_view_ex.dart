...
class InputViewEx extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return AiCartoonInputViewInner(
      padding: padding,
      canInput: canInput,
...
class AiCartoonInputViewInner extends StatefulWidget {
  ...
  State<AiCartoonInputViewInner> createState() =>
      _AiCartoonInputViewInnerState();
}

class _AiCartoonInputViewInnerState extends State<AiCartoonInputViewInner> {
  late TextEditingController controller;
  late final FocusNode focusNode = FocusNode();
  final int maxWords = 2000;

  final wordsController = Get.find<ProhitedWordsController>();

  late final WordsCounter counterController;
  ...
  @override
  void initState() {
    controller = TextEditingController(text: wordsController.inputValue.value)
      ..addListener(_textChanged);

    counterController = Get.put(WordsCounter(
      initialValue: wordsController.inputValue.value.length,
    ));
  ...
  _focusNodeStatusChanged() {
    if (!focusNode.hasFocus) {
      wordsController.inputValue.value = controller.text;
    }
  }
  ...
  Widget buildTextArea(BuildContext context) {
    return Obx(() {
                ...
                child: ListView(
                  ...
                  children: [
                    ByWidgetsUtil.commonText(
                      text: wordsController.inputValue.value,
                      fontSize: 14.sp,
      ...
      controller.text = wordsController.inputValue.value;
      return Container(
            ...
            child: Row(
              ...
              children: [
                ...
                _buildIconBtn(
                  ...
                  onClick: () {
                    controller.text = "";
                    wordsController.inputValue.value = "";
                  },
                ),
                SizedBox(width: 5.w),
              ],
...
class WordsCounterView extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final controller = Get.find<WordsCounter>();
    return Container(
      height: 29.h,
      alignment: Alignment.center,
      padding: EdgeInsets.symmetric(horizontal: 10.w),
      child: Obx(() => ByWidgetsUtil.commonText(
            text: "${controller.count.value}/$maxWords",
            fontSize: 12.sp,
            textColor: ByColorUtil.CommonTextColor.withOpacity(0.5),
          )),
    );
...
class WordsCounter extends GetxController {
  WordsCounter({
    int initialValue = 0,
  }) {
