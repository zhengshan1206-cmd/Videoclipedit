...
import 'package:video_clip_edit/beans/permission_usage_bean.dart';
...
class ByCommonUtils {
  ...
  /// 确保 photo_manager 插件内部权限已正确初始化（解决部分机型授权后仍无法加载相册的问题）
  static Future<bool> _ensurePhotoManagerPermission() async {
    final ps = await PhotoManager.requestPermissionExtend();
    return ps.isAuth || ps.hasAccess;
  }

  /// 获取权限说明内容
  static List<PermissionUsageBean> _getPermissionUsageBeans(RequestType? type) {
    switch (type) {
      case RequestType.image:
        return Permission.photos.permissionUsageBean;
      case RequestType.video:
        return Permission.videos.permissionUsageBean;
      case RequestType.common:
      default:
        // 通用类型需要相册和相机权限说明
        return [
          ...Permission.photos.permissionUsageBean,
          ...Permission.camera.permissionUsageBean,
        ];
    }
  }
  ...
  static pickAssets(
    ...
    // 先显示权限说明弹窗
    final permissionBeans = _getPermissionUsageBeans(type);
    final permissionResult = await showDialog<bool>(
      context: context,
      builder: (ctx) {
        return PermissionsUsageDialog(permissionBeans: permissionBeans);
      },
    );
            ...
            if (idx == 0) {
              switch (type) {
                ...
                case RequestType.image:
                  ...
                  break;
                case RequestType.common:
                  final s1 = await ByPermissionUtils.photos();
                  if (!s1) return;
                  final s2 = await ByPermissionUtils.videos();
                  if (!s2) return;
                  break;
                default:
                  break;
  ...
  static pickAssetsForVideoByType(
    ...
    showDialog(
      ...
      useSafeArea: false,
      builder: (ctx) {
        return AssetsPickerForVideoDialog(
          albumTitle: type == RequestType.audio ? "音频" : null,
          onSelected: (int idx) async {
            ByNavRouterUtils.goBack(ctx);
            if (idx == 0) {
  ...
  static debugPrintObj(Object? object, {String? tag = "VCE ======= :\n"}) {
  ...
  static void exportVideoPage(
    ...
    try {
      ByNavRouterUtils.push(
        context,
        VideoClipHyberPrevicew(path, title: title),
      );
    } catch (e) {
      byDebugPrint(e);
    }
  ...
  static subFunctionCase(
    ...
    if (data.type == 1 || data.type == 2) {
      /// 路由跳转
      // 对于 anime 路由，传递完整的数据对象（包含列表数据）
      if (url == "anime" && extraData != null) {
        RouteUtils.gotoPage(
          context,
          "/$url",
          params: {"id": params, "listData": jsonEncode(extraData)},
        );
      } else {
