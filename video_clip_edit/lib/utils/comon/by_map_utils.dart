...
class ByMapUtils {
  ...
  static void openAMapNavi({
    ...
    String dName = '', // 终点名称
    String dev =
        '0', // 起终点是否偏移。0:lat和lon是已经加密后的,不需要国测加密;1:需要国测加密，可为空，但起点或终点不为空时，不能为空
    String t =
        '0', // t = 0 驾车；    t = 1 公交；    t = 2 步行；    t = 3 骑行（骑行仅在V788以上版本支持）
    String message = '您没有安装高德地图，请先安装高德地图！',
  }) async {
    if (!ByPackageUtils.isMobile) {
      return;
    }
    var type = ByPackageUtils.isIOS
    ...
    if (!await isInstallMap(url)) {
      ByProgressHUD.showText(message);
      return;
    }
  ...
  static void openAMapNavi2({
    ...
    var device = ByPackageUtils.isAndroid ? 'android' : 'ios';
    var url =
        '${device}amap://navi?sourceApplication=amap&poiname=$poiName&lat=$latitude&lon=$longitude&dev=$dev';
    if (!await isInstallMap(url)) {
      ByProgressHUD.showText(message);
      return;
    }
  ...
  static void openBaiduMapNavi({
    ...
    String dName = '', // 终点名称
    String mode =
        'driving', // 导航模式，可选transit（公交）、driving（驾车）、walking（步行）和riding（骑行）默认:driving
    String coordType =
  ...
  static void openTencentMapNavi({
    ...
    var url =
        'qqmap://map/routeplan?type=$type&$formInfo&to=$dName&tocoord=$dLatitude,$dLongitude&referer=$referer';
    if (!await isInstallMap(url)) {
      ByProgressHUD.showText(message);
      return;
    }
