...
class BatchReplaceDialog<T extends MaterialBaseProvider>
    extends StatefulWidget {
  const BatchReplaceDialog({super.key});

  @override
  State<BatchReplaceDialog<T>> createState() => _BatchReplaceDialogState<T>();
}

class _BatchReplaceDialogState<T extends MaterialBaseProvider>
    extends State<BatchReplaceDialog<T>> {
                  ...
                  onClick: () {
                    ...
                    provider.replaceWord(wordsEditingController.text, () {
                      provider.detect(
                        ...
                        onSuccess: () {
                          if (provider.prohibiteWords.isEmpty) {
  ...
  _buildProhibitedWords(BuildContext context) {
    return Consumer<T>(builder: (context, provider, child) {
      ...
      byDebugPrint(prohibiteWords, tag: "prohibiteWords:");
    ...
    return Container(
        ...
        child: Consumer<T>(builder: (context, provider, child) {
          return ByWidgetsUtil.commonText(
            text: provider.selectedProhibiteWord,
            textColor: ByColorUtil.CommonTextColor,
            fontSize: 16.sp,
          );
        }));
