...
class CloudMaterialCell<T extends MaterialBaseProvider> extends StatefulWidget {
  const CloudMaterialCell({
    super.key,
    required this.index,
  ...
  @override
  State<CloudMaterialCell<T>> createState() => _CloudMaterialCellState<T>();
}

class _CloudMaterialCellState<T extends MaterialBaseProvider>
    extends State<CloudMaterialCell<T>> {
  ...
  Widget build(BuildContext context) {
    ...
    final selectedUrls =
        provider.selectedVideoDetailBeans.map((e) => e.videoUrl).toList();
    final currentUrl = widget.videoBean.videoUrl;
    bool selected = selectedUrls.contains(currentUrl);
    return GestureDetector(
      behavior: HitTestBehavior.opaque,
      onTap: () async {
        if (widget.fromMyWorks && !fileExists) return;
        final selectedCout = provider.selectedVideoDetailBeans.length;
        ...
        provider.updateSelectedVideoDetailBeans(widget.videoBean);
        if (widget.fromMyWorks) {
          /// 刷新页面
          context.read<MinePageProvider>().refresh();
        }
      },
      child: ClipRRect(
