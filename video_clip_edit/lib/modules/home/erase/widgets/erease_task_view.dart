...
enum EraseTaskViewType { picture, video }

class EraseTaskView extends StatelessWidget {
  final EreaseRecordBean myWorkBean;
  final EraseTaskViewType? type;
  const EraseTaskView({
    super.key,
  ...
  Widget build(BuildContext context) {
    return ClipRRect(
      ...
      child: Stack(
        children: [
          Positioned.fill(
            child: myWorkBean.url.isEmpty
                ? Container()
                : CachedNetworkImage(
                    fit: BoxFit.cover,
                    imageUrl: myWorkBean.url,
                  ),
          ),
          Positioned.fill(
            child: Offstage(
              offstage: myWorkBean.status != 3,
              child: Container(
          ...
          Positioned.fill(
            child: Offstage(
              offstage: type == EraseTaskViewType.picture,
              child: Container(
                alignment: Alignment.center,
                child: SizedBox(
                  width: 40.w,
